...
const CounterGasless: React.FC<Props> = ({ smartAccount, provider }) => {
 ...
 const counterAddress = "0x301e21fe3e556c3da730eda17281cde0d3bcb723" ;
  ...
  const getCount = async (isUpdating: boolean) => {
    ...
    if (isUpdating) {
      toast.success('Congrats, your count got updated using Sponsorship Paymaster !', {
        position: "top-right",
        autoClose: 5000,
  ...
  return (
    <>
      ...
      />
      <br></br>
      <button onClick={() => incrementCount()}>Increment Count using Sponsored (Gasless) Paymaster</button>
    </>
  );
};

export default CounterGasless;
